import { z } from 'zod'
import { idSchema, paginationMetaSchema, paginationSchema, searchSchema } from './shared.js'

// Enums
export const EventTypeEnum = z
  .enum(['SERVICE', 'CELL', 'MINISTRY', 'OTHER'])
  .describe('Tipo do evento')
export const EventStatusEnum = z
  .enum(['SCHEDULED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED'])
  .describe('Status do evento')
export const ParticipantStatusEnum = z
  .enum(['CONFIRMED', 'PENDING', 'CANCELLED'])
  .describe('Status do participante')
export const EventSortByEnum = z
  .enum(['title', 'startDate', 'type', 'createdAt'])
...
export const eventSchema = z
  .object({
    ...
    recurrenceRule: z.enum(['WEEKLY', 'MONTHLY']).nullable(),
    recurrenceEndDate: z.coerce.date().nullable(),
    recurrenceExceptions: z.array(z.coerce.date()),
...
const baseEventSchema = z
  .object({
    ...
    recurrenceExceptions: z.array(z.coerce.date()).optional(),
...
const validateEventDates = (data: {
  ...
  if (data.startDate && data.recurrenceEndDate && data.startDate > data.recurrenceEndDate) {
    return false
  }
  return true
}
...
export function serializeRecurrenceExceptions(exceptions: Date[] | undefined) {
  return JSON.stringify((exceptions ?? []).map((date) => date.toISOString()))
}

export function parseEventResponse(event: {
  recurrenceExceptions?: string | null
  [key: string]: unknown
}) {
  let recurrenceExceptions: unknown[] = []
  try {
    const parsed = JSON.parse(event.recurrenceExceptions || '[]')
    if (Array.isArray(parsed)) recurrenceExceptions = parsed
  } catch {
    recurrenceExceptions = []
  }
  return eventSchema.parse({ ...event, recurrenceExceptions })
}
