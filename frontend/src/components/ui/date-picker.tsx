...
export function DatePickerInput({ label, className, value, onChange }: DatePickerProps) {
  const now = today(getLocalTimeZone());

  return (
    <DatePicker className={cn("space-y-2", className)} value={value} onChange={onChange}>
      {label && <Label className="text-sm font-medium text-foreground">{label}</Label>}
      <div className="relative group">
        <div className="flex">
          <Group className={cn(
            "inline-flex h-10 w-full items-center overflow-hidden whitespace-nowrap rounded-md px-3 py-2 pe-9 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50",
            "transition-all duration-300 bg-background/50 border border-primary/20",
            "group-focus-within:bg-background/90 group-focus-within:border-primary"
          )}>
            <DateInput>
              {(segment) => (
                <DateSegment
                  segment={segment}
                  className="inline rounded p-0.5 text-foreground caret-transparent outline outline-0 data-[disabled]:cursor-not-allowed data-[focused]:bg-accent data-[invalid]:data-[focused]:bg-destructive data-[invalid]:data-[focused]:data-[placeholder]:text-destructive-foreground data-[invalid]:data-[focused]:text-destructive-foreground data-[invalid]:data-[placeholder]:text-destructive data-[invalid]:text-destructive data-[placeholder]:text-muted-foreground/70 data-[type=literal]:text-muted-foreground/70 data-[disabled]:opacity-50"
                />
              )}
            </DateInput>
          </Group>
